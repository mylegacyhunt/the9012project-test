 ...
 function mount(root){
  ...
  function install(){
   const nav=doc.querySelector('#viewWisdom .wisdomnav');if(nav){const tab=btn('◆ Golden Nuggets',openNuggets);tab.id='ksNuggetsTab';nav.prepend(tab);
    ...
    const section=node('section',undefined,'wisdomsection');section.id='ksNuggetsSection';const art=node('img',undefined,'ks-nugget-art');art.src='assets/images/shared/other/golden-nugget-app.png';art.alt='Golden Nugget';const list=node('div',undefined,'ks-list');list.id='ksNuggetsList';section.append(art,note('Truth Worth Keeping — a sentence, quotation, or insight drawn from life. Keep it private or choose to share it with family.'),btn('+ Add a Golden Nugget',()=>nuggetEditor()),list);nav.after(section);
   }
